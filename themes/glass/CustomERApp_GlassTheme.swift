...
// MARK: - Modern Cyber Dark Glass Theme Tokens
...
let kAddBg       = NSColor(red: 16/255.0,  green: 185/255.0, blue: 129/255.0, alpha: 1.00) // #10B981 Emerald Cyber Green
let kDim         = NSColor(red: 100/255.0, green: 116/255.0, blue: 139/255.0, alpha: 1.00) // #64748B Steel Slate
let kSurface     = NSColor(red: 21/255.0,  green: 29/255.0,  blue: 42/255.0,  alpha: 1.00) // #151D2A Frosted Glass Card Surface
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            for url in files {
                ...
                if FileManager.default.fileExists(atPath: url.path, isDirectory: &isDir), isDir.boolValue, url.pathExtension != "app" {
                    let icon = generateFolderIcon(folderName: url.lastPathComponent, bgColor: bgColor, accentColor: kBlue)
...
class ReminderRowView: NSView {
    ...
    private func build() {
        ...
        check.wantsLayer = true; check.layer?.cornerRadius = 6; check.layer?.borderWidth = kBorderWidth; check.layer?.borderColor = kBorder.cgColor
