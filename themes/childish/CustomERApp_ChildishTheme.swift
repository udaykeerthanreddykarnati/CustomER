...
// MARK: - Global Theme Colors (Bubbly Childish)
var kBg          = NSColor(red: 255/255.0, green: 227/255.0, blue: 241/255.0, alpha: 1.00) // #FFE3F1 Cotton Candy Pink
let kBlue        = NSColor(red: 255/255.0, green: 79/255.0,  blue: 160/255.0, alpha: 1.00)  // Bubblegum Pop header & text (#FF4FA0)
...
let kAddBg       = NSColor(red: 255/255.0, green: 79/255.0,  blue: 160/255.0, alpha: 1.00)  // Bubblegum Pop (#FF4FA0)
let kDim         = NSColor(red: 0.55, green: 0.42, blue: 0.62, alpha: 1.00)            // Dusty lavender text
let kSurface     = NSColor(red: 255/255.0, green: 246/255.0, blue: 250/255.0, alpha: 1.00) // #FFF6FA Marshmallow Cream
let kBorder      = NSColor(red: 0.00, green: 0.00, blue: 0.00, alpha: 1.00)            // Solid Black Cartoon Outline
let kRadius: CGFloat = 16.0 // bubbly rounded corners used across every card, button, and badge
...
class FolderIconManager {
    ...
    private static func generateFolderIcon(folderName: String, bgColor: NSColor, accentColor: NSColor) -> NSImage {
        ...
        let attr: [NSAttributedString.Key: Any] = [
            .font: roundedFont(size: 32, weight: .black),
...
class ReminderRowView: NSView {
    ...
    private func build() {
        wantsLayer = true; layer?.cornerRadius = kRadius; layer?.backgroundColor = kSurface.cgColor
        layer?.borderWidth = 0.0

        check.isBordered = false; check.bezelStyle = .regularSquare; check.setButtonType(.momentaryChange)
        check.wantsLayer = true; check.layer?.cornerRadius = kRadius; check.layer?.borderWidth = 1.5; check.layer?.borderColor = kBorder.cgColor
        check.layer?.backgroundColor = reminder.isCompleted ? kBorder.cgColor : NSColor.clear.cgColor
        if reminder.isCompleted {
            check.attributedTitle = NSAttributedString(string: "✓", attributes: [.font: roundedFont(size: 10, weight: .bold), .foregroundColor: NSColor.white])
        } else { check.title = "" }
        ...
        label.font = roundedFont(size: 13, weight: .medium); label.lineBreakMode = .byTruncatingTail
        if reminder.isCompleted {
        ...
        del.title = "✕"; del.font = roundedFont(size: 10, weight: .bold); del.isBordered = false
...
class AddPanel: NSView {
    ...
    private func build() {
        ...
        titleField.font = roundedFont(size: 14, weight: .bold); titleField.focusRingType = .none
        titleField.isBordered = true; titleField.bezelStyle = .squareBezel; titleField.wantsLayer = true
        titleField.layer?.cornerRadius = kRadius; titleField.layer?.borderWidth = 2; titleField.layer?.borderColor = kBorder.cgColor

        let pAttrs: [NSAttributedString.Key: Any] = [.foregroundColor: NSColor(white: 0.45, alpha: 1.0), .font: roundedFont(size: 13, weight: .medium)]
    ...
    private func styleBtn(_ b: NSButton, primary: Bool, title: String) {
        b.title = title; b.isBordered = false; b.wantsLayer = true; b.layer?.cornerRadius = kRadius
        b.layer?.borderWidth = 1.5; b.layer?.borderColor = kBorder.cgColor; b.font = roundedFont(size: 12, weight: .bold)
...
class TodoView: NSView {
    ...
    private func build() {
        ...
        header.font = roundedFont(size: 17, weight: .bold); header.textColor = kBlue
        header.isEditable = false; header.isBordered = false; header.backgroundColor = .clear
        addSubview(header)

        badge.font = roundedFont(size: 10, weight: .bold); badge.textColor = kBlue; badge.alignment = .center
        badge.isEditable = false; badge.isBordered = false; badge.backgroundColor = .clear; badge.wantsLayer = true
        badge.layer?.cornerRadius = kRadius; badge.layer?.borderWidth = 1; badge.layer?.borderColor = kBorder.cgColor
        ...
        addBtn.title = "+ Add"; addBtn.font = roundedFont(size: 12, weight: .bold); addBtn.isBordered = false
        addBtn.wantsLayer = true; addBtn.layer?.cornerRadius = kRadius; addBtn.layer?.backgroundColor = kAddBg.cgColor
        addBtn.contentTintColor = .white; addBtn.target = self; addBtn.action = #selector(showPanel); addSubview(addBtn)

        clearBtn.title = "Clear Done"; clearBtn.font = roundedFont(size: 11, weight: .medium)
    ...
    private func styleTabButton(_ btn: NSButton, title: String, isActive: Bool) {
        btn.isBordered = false; btn.wantsLayer = true; btn.layer?.cornerRadius = kRadius
        ...
        btn.attributedTitle = NSAttributedString(string: title, attributes: [
            .font: roundedFont(size: 11, weight: isActive ? .bold : .medium),
...
class CalendarView: NSView {
    ...
    private func build() {
        ...
        titleLabel.font = roundedFont(size: 13, weight: .black); titleLabel.textColor = kText
        ...
        for h in headers {
            let lbl = NSTextField(labelWithString: h); lbl.font = roundedFont(size: 10, weight: .black); lbl.textColor = kDim; lbl.alignment = .center
            addSubview(lbl); dayViews.append(lbl)
        }

        for _ in 0..<35 {
            let lbl = NSTextField(); lbl.font = roundedFont(size: 11, weight: .bold); lbl.alignment = .center
            lbl.isEditable = false; lbl.isBordered = false; lbl.backgroundColor = .clear; lbl.wantsLayer = true; lbl.layer?.cornerRadius = kRadius
            addSubview(lbl); dayViews.append(lbl)
        }
        updateCalendar()
    }
    ...
    private func updateCalendar() {
        ...
        for i in 0..<35 {
            ...
            if dNum >= 1 && dNum <= numDays {
                ...
                if dNum == today {
                    lbl.backgroundColor = kText; lbl.textColor = .white; lbl.font = roundedFont(size: 11, weight: .black)
                } else {
                    lbl.backgroundColor = .clear; lbl.textColor = kText; lbl.font = roundedFont(size: 11, weight: .bold)
                }
...
class BatteryView: NSView {
    ...
    private func build() {
        ...
        macCard.wantsLayer = true; macCard.layer?.cornerRadius = kRadius; macCard.layer?.backgroundColor = kSurface.cgColor
        macCard.layer?.borderWidth = 1.5; macCard.layer?.borderColor = kBorder.cgColor; addSubview(macCard)

        macPctLabel.font = roundedFont(size: 32, weight: .black); macPctLabel.textColor = kText
        ...
        statusCell.alignment = .center; statusCell.font = roundedFont(size: 11, weight: .bold); statusCell.textColor = .white
        macStatusLabel.cell = statusCell; macStatusLabel.isEditable = false; macStatusLabel.isBordered = false
        macStatusLabel.wantsLayer = true; macStatusLabel.layer?.cornerRadius = kRadius; macStatusLabel.layer?.backgroundColor = kAccent.cgColor
        macStatusLabel.layer?.borderWidth = 1.5; macStatusLabel.layer?.borderColor = kBorder.cgColor; macCard.addSubview(macStatusLabel)

        macProgressBar.wantsLayer = true; macProgressBar.layer?.cornerRadius = kRadius; macProgressBar.layer?.backgroundColor = NSColor.white.cgColor
        ...
        btHeaderLabel.font = roundedFont(size: 10, weight: .bold); btHeaderLabel.textColor = kDim
    ...
    private func renderBTDevices(_ devices: [(name: String, battery: Int?)]) {
        ...
        if devices.isEmpty {
            ...
            noDevLabel.font = roundedFont(size: 11, weight: .medium); noDevLabel.textColor = kDim; noDevLabel.alignment = .center
        ...
        for (i, dev) in devices.enumerated() {
            ...
            devCard.wantsLayer = true; devCard.layer?.cornerRadius = kRadius; devCard.layer?.backgroundColor = kSurface.cgColor
            ...
            nameCell.font = roundedFont(size: 12, weight: .bold); nameCell.textColor = kText; nameCell.lineBreakMode = .byTruncatingTail
            ...
            badgeCell.alignment = .center; badgeCell.font = roundedFont(size: 11, weight: .bold); badgeCell.textColor = .white
            badgeTF.cell = badgeCell; badgeTF.stringValue = "\(batVal)%"
            badgeTF.isEditable = false; badgeTF.isBordered = false
            badgeTF.wantsLayer = true; badgeTF.layer?.cornerRadius = kRadius; badgeTF.layer?.backgroundColor = kAccent.cgColor
...
class DigitalClockView: NSView {
    ...
    private func build() {
        ...
        timeLabel.font = roundedFont(size: 42, weight: .black); timeLabel.textColor = kText; timeLabel.alignment = .center
        timeLabel.isEditable = false; timeLabel.isBordered = false; timeLabel.backgroundColor = .clear; addSubview(timeLabel)

        dateLabel.font = roundedFont(size: 12, weight: .bold); dateLabel.textColor = kDim; dateLabel.alignment = .center
...
class ColorPickerView: NSView {
    ...
    private var dragStart: NSPoint = .zero, dragActive = false
    private let presetHexes = ["#FFB3D9", "#A8E6FF", "#FFF4A3", "#D9B3FF", "#B3FFD9", "#FFCBA4"]
    ...
    private func build() {
        ...
        pickerBtn.font = roundedFont(size: 11, weight: .bold)
        pickerBtn.isBordered = false; pickerBtn.wantsLayer = true; pickerBtn.layer?.cornerRadius = kRadius
    ...
    private func renderSwatches() {
        ...
        for (i, hex) in presetHexes.enumerated() {
            ...
            btn.title = ""; btn.isBordered = false; btn.wantsLayer = true; btn.layer?.cornerRadius = kRadius
...
class SpotifyView: NSView {
    ...
    private func build() {
        ...
        artImageView.wantsLayer = true; artImageView.layer?.cornerRadius = kRadius; artImageView.layer?.borderWidth = 1.5; artImageView.layer?.borderColor = kBorder.cgColor
        artImageView.imageScaling = .scaleAxesIndependently; artImageView.imageAlignment = .alignCenter; addSubview(artImageView)

        trackLabel.font = roundedFont(size: 13, weight: .black); trackLabel.textColor = kText; trackLabel.isEditable = false; trackLabel.isBordered = false; trackLabel.backgroundColor = .clear; trackLabel.lineBreakMode = .byTruncatingTail; addSubview(trackLabel)
        artistLabel.font = roundedFont(size: 11, weight: .bold); artistLabel.textColor = kDim; artistLabel.isEditable = false; artistLabel.isBordered = false; artistLabel.backgroundColor = .clear; artistLabel.lineBreakMode = .byTruncatingTail; addSubview(artistLabel)

        progressBar.wantsLayer = true; addSubview(progressBar)
        currTimeLabel.font = roundedFont(size: 10, weight: .bold); currTimeLabel.textColor = kText; currTimeLabel.isEditable = false; currTimeLabel.isBordered = false; currTimeLabel.backgroundColor = .clear; addSubview(currTimeLabel)
        durTimeLabel.font = roundedFont(size: 10, weight: .bold); durTimeLabel.textColor = kDim; durTimeLabel.isEditable = false; durTimeLabel.isBordered = false; durTimeLabel.backgroundColor = .clear; durTimeLabel.alignment = .right; addSubview(durTimeLabel)
        ...
        statusLabel.stringValue = "Spotify Offline"; statusLabel.font = roundedFont(size: 12, weight: .bold); statusLabel.textColor = kDim; statusLabel.alignment = .center; statusLabel.isEditable = false; statusLabel.isBordered = false; statusLabel.backgroundColor = .clear; statusLabel.isHidden = true; addSubview(statusLabel)
    }

    private func setupBtn(_ btn: NSButton, title: String, action: Selector) {
        btn.title = title; btn.font = roundedFont(size: 12, weight: .bold); btn.isBordered = false; btn.wantsLayer = true; btn.layer?.cornerRadius = kRadius; btn.layer?.backgroundColor = kText.cgColor; btn.layer?.borderWidth = 1; btn.layer?.borderColor = kBorder.cgColor; btn.contentTintColor = .white; btn.target = self; btn.action = action; addSubview(btn)
    }
...
class GithubView: NSView {
    ...
    private func build() {
        ...
        userBadge.font = roundedFont(size: 10, weight: .bold); userBadge.textColor = kDim; userBadge.alignment = .right
        userBadge.isEditable = false; userBadge.isBordered = false; userBadge.backgroundColor = .clear
        addSubview(userBadge)

        avatarImageView.wantsLayer = true; avatarImageView.layer?.cornerRadius = 41; avatarImageView.layer?.borderWidth = 1.5; avatarImageView.layer?.borderColor = kBorder.cgColor
    ...
    private func setupStatCard(_ card: NSView, numLabel: NSTextField, titleLabel: NSTextField, title: String) {
        card.wantsLayer = true; card.layer?.cornerRadius = kRadius; card.layer?.backgroundColor = kSurface.cgColor
        card.layer?.borderWidth = 1.5; card.layer?.borderColor = kBorder.cgColor; addSubview(card)

        numLabel.stringValue = "-"
        numLabel.font = roundedFont(size: 18, weight: .black); numLabel.textColor = kText; numLabel.alignment = .center
        numLabel.isEditable = false; numLabel.isBordered = false; numLabel.backgroundColor = .clear; card.addSubview(numLabel)

        titleLabel.stringValue = title
        titleLabel.font = roundedFont(size: 10, weight: .bold); titleLabel.textColor = kDim; titleLabel.alignment = .center
...
class MailView: NSView {
    ...
    private func build() {
        ...
        badgeLabel.font = roundedFont(size: 10, weight: .bold); badgeLabel.textColor = kDim; badgeLabel.alignment = .right
        badgeLabel.isEditable = false; badgeLabel.isBordered = false; badgeLabel.backgroundColor = .clear
        addSubview(badgeLabel)

        cardView.wantsLayer = true; cardView.layer?.cornerRadius = kRadius; cardView.layer?.backgroundColor = kSurface.cgColor
        cardView.layer?.borderWidth = 1.5; cardView.layer?.borderColor = kBorder.cgColor; addSubview(cardView)

        countNumLabel.stringValue = "0"
        countNumLabel.font = roundedFont(size: 22, weight: .black); countNumLabel.textColor = kText; countNumLabel.alignment = .center
        ...
        senderLabel.font = roundedFont(size: 11, weight: .bold); senderLabel.textColor = kText; senderLabel.lineBreakMode = .byTruncatingTail
        ...
        subjectLabel.font = roundedFont(size: 10, weight: .medium); subjectLabel.textColor = kDim; subjectLabel.lineBreakMode = .byTruncatingTail
...
class PhotosView: NSView {
    ...
    private func build() {
        ...
        imageView.wantsLayer = true; imageView.layer?.cornerRadius = kRadius; imageView.imageScaling = .scaleAxesIndependently; imageView.imageAlignment = .alignCenter
        addSubview(imageView)
    }
