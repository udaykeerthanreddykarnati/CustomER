...
extension NSColor {
    ...
    func toHex() -> String {
        guard let rgb = usingColorSpace(.sRGB) else { return "#0B0F19" }
...
// MARK: - Apple Dark Glass Theme Design Tokens
var kBg      = NSColor(hex: "#0B0F19")!.withAlphaComponent(0.85) // Dark Glass Midnight Tint
let kText    = NSColor.white                      // Crisp Pure White (#FFFFFF)
let kBlue    = NSColor(hex: "#3B82F6")!           // Apple Electric Blue (#3B82F6)
let kAccent  = NSColor(hex: "#3B82F6")!           // Apple Electric Blue (#3B82F6)
let kAddBg   = NSColor(hex: "#3B82F6")!           // Apple Electric Blue (#3B82F6)
let kDim     = NSColor(white: 1.0, alpha: 0.65)   // Muted Glass White Text
let kSurface = NSColor(white: 1.0, alpha: 0.12)   // Apple Translucent Dark Glass Surface Opacity
let kBorder  = NSColor(white: 1.0, alpha: 0.20)   // Translucent Glass Border Highlight
let kRadius: CGFloat = 20.0                       // Apple Smooth 20px Glass Pill Shape
let kBorderWidth: CGFloat = 1.0                   // 1.0px Subtle Border
...
class ThemeManager {
    ...
    private let themeFileURL: URL = {
        ...
        return dir.appendingPathComponent("glass_theme.json")
    }()
    ...
    var currentBgColor: NSColor {
        get {
            ...
            return NSColor(hex: "#0B0F19")!.withAlphaComponent(0.85)
        }
        set {
            ...
            kBg = newValue.withAlphaComponent(0.85)
...
class GithubView: NSView {
    ...
    private var username: String = "udaykeerthanreddykarnati"
    ...
    private func build() {
        ...
        userBadge.font = dynamicFont(size: 10, weight: .bold); userBadge.textColor = kDim; userBadge.alignment = .right
    ...
    private func setupStatCard(_ card: NSView, numLabel: NSTextField, titleLabel: NSTextField, title: String) {
        card.wantsLayer = true; card.layer?.cornerRadius = kRadius / 2; card.layer?.backgroundColor = kSurface.cgColor
        card.layer?.borderWidth = kBorderWidth; card.layer?.borderColor = kBorder.cgColor; addSubview(card)

        numLabel.stringValue = "-"
        numLabel.font = dynamicFont(size: 18, weight: .bold); numLabel.textColor = kText; numLabel.alignment = .center
        numLabel.isEditable = false; numLabel.isBordered = false; numLabel.backgroundColor = .clear; card.addSubview(numLabel)

        titleLabel.stringValue = title
        titleLabel.font = dynamicFont(size: 10, weight: .bold); titleLabel.textColor = kDim; titleLabel.alignment = .center
    ...
    private func fetchStats() {
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            let ghLogin = shellRun("gh api user -q .login 2>/dev/null").trimmingCharacters(in: .whitespacesAndNewlines)
            let login = ghLogin.isEmpty ? "udaykeerthanreddykarnati" : ghLogin
            let token = shellRun("gh auth token 2>/dev/null").trimmingCharacters(in: .whitespacesAndNewlines)
            
            DispatchQueue.main.async {
                self?.username = login
                self?.userBadge.stringValue = "@\(login)"
            }

            if let url = URL(string: "https://api.github.com/users/\(login)") {
            ...
            if !token.isEmpty, let gqlUrl = URL(string: "https://api.github.com/graphql") {
                ...
                let query = ["query": "query { user(login: \"\(login)\") { contributionsCollection { contributionCalendar { totalContributions } } } }"]
            ...
            } else if let cUrl = URL(string: "https://github.com/users/\(login)/contributions") {
